-- Migration: Adaptive Search and Learning-to-Rank Schema (v2 - Fixed Foreign Keys)
-- This migration adds tables for adaptive quantile thresholds, user feedback, and ranking features
-- with proper foreign key relationships to the users and emails tables

-- 1. Query Type Quantiles Table
-- Stores rolling quantile statistics per query type (global, not per-user)
...
-- FIXED: Now properly references users table
CREATE TABLE IF NOT EXISTS search_queries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,  -- FK to users.id (UUID)
    google_user_id VARCHAR(255),  -- Denormalized for backwards compatibility
...
-- FIXED: Now properly references users and emails tables
CREATE TABLE IF NOT EXISTS search_feedback (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    search_query_id UUID REFERENCES search_queries(id) ON DELETE CASCADE,
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,  -- FK to users.id (UUID)
    google_user_id VARCHAR(255),  -- Denormalized for backwards compatibility
    email_id UUID,  -- FK to emails.id (UUID) - but emails table might not have UUID id yet
    thread_id VARCHAR(255),  -- Gmail thread ID for lookup
...
CREATE TABLE IF NOT EXISTS user_search_preferences (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,  -- FK to users.id (UUID)
    google_user_id VARCHAR(255) NOT NULL,  -- Denormalized for backwards compatibility
    ...
    UNIQUE(google_user_id)  -- Also unique on google_user_id for backwards compat
);
...
CREATE TABLE IF NOT EXISTS sender_affinity (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(user_id, sender_email)  -- Use user_id instead of google_user_id
);
...
CREATE INDEX IF NOT EXISTS idx_search_queries_google_user_id ON search_queries(google_user_id);
...
CREATE INDEX IF NOT EXISTS idx_search_feedback_google_user_id ON search_feedback(google_user_id);
...
CREATE INDEX IF NOT EXISTS idx_sender_affinity_user_id ON sender_affinity(user_id);
CREATE INDEX IF NOT EXISTS idx_sender_affinity_google_user_id ON sender_affinity(google_user_id);
...
-- IMPORTANT NOTES:
-- 1. This schema assumes the 'users' table has a UUID primary key called 'id'
-- 2. This schema assumes the 'users' table has a 'google_user_id' VARCHAR column
-- 3. We keep google_user_id denormalized for backwards compatibility and query performance
-- 4. The emails table relationship is looser (via thread_id) since emails table structure varies
